...
class mqtt_connection(object):
    ...
    def __init__(self,url,port,token,listen):
        self.MQTTHOST = url
        self.MQTTPORT = port
        self.token = token
        self.mqttClient = mqtt.Client(client_id='tornado_server'+'_'+token)
        self.msg_group = {}
        self.listen = listen[:-3]+'rsp'
        self.result = {
            self.token:None
        }
    ...
    def on_message_come(self, client, userdata, msg):
        ...
        if len(items) > 4 and items[4] == self.listen:
            token = items[5]
    ...
    def publish_mqtt_msg(self,topic, msg, qos=0):
        ...
        if result != mqtt.MQTT_ERR_SUCCESS:
            ...
            return False
        dbg("success result %d, mid %d" % (result, mid))
        return True
    ...
    def on_subscribe(self):
        self.mqttClient.subscribe("/v1/device/+/"+self.listen+"/+")
        dbg('加入监听频道，监听机器回调')
